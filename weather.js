...
let handleWeatherResponse = function(response) {
  ...
  for (i = 0; i < 6; i++) {
    ...
    markup += "<h4>" + icon(response.daily.data[i]) + "</h4>";
    markup += "<h4>" + "Low " + response.daily.data[i].apparentTemperatureLow + " | " + "High " + response.daily.data[i].apparentTemperatureHigh + "</h4>";
    markup += "<h4>" + response.daily.data[i].summary + "</h4>";
    markup += "</div>";
  }
